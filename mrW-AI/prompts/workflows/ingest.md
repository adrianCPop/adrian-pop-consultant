...
- **FINDINGS [CRITICAL]**: Create or update `mrW-AI/findings.md` with observations. Follow `prompts/instructions/shared-context.md` write rules (ACTIVE/ARCHIVE format).
...
## Phase 0.5: select-profile

Ask the user:

> "How much structure do you want?
> - **Minimal** — CLAUDE.md + constraints + builder agent. Solo/prototype.
> - **Standard** — Adds reviewer, /mrw-build, /mrw-review, shared state, templates.
> - **Full** — Adds all agents, all commands, hooks, tasks, skills. Complex/long projects."
>
> Recommended: Standard for most projects. Use Minimal for quick prototypes, Full for complex multi-sprint work.

Each profile generates:

**Minimal:**
- CLAUDE.md, mrW-AI/architecture.md, conventions.md, stack.md
- mrW-AI/shared/constraints/global.md + domain constraint files
- mrW-AI/shared/glossary.md
- mrW-AI/agents/builder.md, .claude/agents/builder.md
- .mcp.json + .mcp.json.example

**Standard (adds to Minimal):**
- mrW-AI/agents/reviewer.md + .claude/agents/reviewer.md
- mrW-AI/shared/findings-index.md, decisions-index.md + archive dirs
- mrW-AI/shared/templates/ (finding, decision, task, rejection-log, jira-description)
- .claude/commands/mrw-build.md, mrw-review.md

**Full (adds to Standard):**
- All remaining agents (planner, architect, smoke-tester, docs-agent)
- All commands (mrw-plan, mrw-run, mrw-check-contracts)
- Hooks (constraint validator, process cleanup)
- mrW-AI/tasks/backlog.md, mrW-AI/skills/index.md

Store the selected profile in session memory. Use it to gate which files are generated in Phase 4 (create-scaffolding).
...
Read `prompts/instructions/shared-context.md` for the ACTIVE/ARCHIVE format and write rules.

**Profile gating:** If profile is Minimal, skip Standard and Full files. If profile is Standard, skip Full-only files.
...
- `mrW-AI/shared/decisions.md` — use ACTIVE/ARCHIVE format. Add DECISION-0 (pre-existing architecture) to both Active (one-line) and Archive (full detail)
- `mrW-AI/shared/glossary.md` — domain terms table from extraction
- `mrW-AI/findings.md` — use ACTIVE/ARCHIVE format. Add critical observations as one-liners in Active, full detail in Archive
- `mrW-AI/mrw-plans.yml` — empty plan registry (`plans: []`)

## Phase 4.5: generate-codex-snapshot

Read `prompts/instructions/codex-snapshot.md` for the format spec.

Generate `mrW-AI/codex.md` from the repo-analyst output already available from Phase 2.
Do NOT spawn an extra agent — extract and compress the analysis into the codex format:

- Stack: language, framework, package manager, build/test/lint commands
- Structure: 2-level directory tree (pruned)
- Key Modules: top directories with purpose (tab-separated, max 20)
- Exports: public API surface (tab-separated, max 15)
- Schema: DB models if detected (tab-separated, max 15)
- Routes: HTTP endpoints if detected (tab-separated, max 20)
- Test Infrastructure: runner, framework, pattern, location

Skip empty sections. Target 50-100 lines total.
...
Read back each created/updated file to confirm correctness — including `mrW-AI/codex.md`.

Report to user:
- Files created: [list with paths]
- Conventions documented: [count]
- Domain terms captured: [count]
- Codex snapshot: [line count] lines
