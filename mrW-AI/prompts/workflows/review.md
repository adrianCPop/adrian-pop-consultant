...
- **CONSTRAINTS-CHECK [HIGH]**: Verify compliance with `mrW-AI/shared/constraints.md` rules (read in full).
...
Read `mrW-AI/codex.md` for codebase overview (if it exists).
Read original spec and `mrW-AI/tasks/current.md`.
...
```markdown
## Review: {task_prefix}-NNN — Task Title
