...
| `conflict-resolve` | main Codex session | `conflict-resolver` | `worker` | `gpt-5.4` |
...
- Resolves git merge conflicts using task context and change-intent manifests
- Reads done/ tasks and change-intents/ to attribute "theirs" side changes
- Writes resolved source files via git add, decisions if non-obvious
- Default model: `gpt-5.4`
