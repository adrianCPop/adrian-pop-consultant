...
- `mrW-AI/tasks/{task_prefix}-NNN.md` _(Read `task_prefix` from `config/multi-dev.yml`; default: `MRW`)_
...
- conflicting files (from git merge state)
- `mrW-AI/tasks/current.md` — "ours" task intent
- `mrW-AI/tasks/done/` — "theirs" task files
...
- resolved source files (staged via `git add`)
- `mrW-AI/shared/decisions.md` when non-obvious resolution choices are made

Returns:

- resolution summary with per-file confidence levels
- test results post-resolution
- unresolved files with reasons
- attribution quality warnings
